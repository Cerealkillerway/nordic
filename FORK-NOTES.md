...
The splash uses the same artwork as the login screen, as
`extras/plymouth/nordic/background.png`. It is rendered at 1920x1080 rather
than 2560x1440 deliberately: the file goes into the initramfs, and the theme
scales it at runtime anyway. It adds roughly 1.4 MB there. The script scales it
to *cover* the screen — `Image.Scale` takes an explicit width and height, so
handing it the screen size would stretch the picture on any aspect ratio other
than the one it was rendered at.
...
#    plymouthd has NO --renderer flag; it picks one from the plugins present.
#    Installing plymouth-x11 puts x11.so alongside drm.so, and -E keeps DISPLAY
#    and XAUTHORITY so the X11 renderer is usable.
sudo apt install plymouth-x11
sudo -E plymouthd --debug --debug-file=/tmp/plymouth.log
sudo -E plymouth --show-splash
sleep 15 ; sudo plymouth --quit
#    the LUKS prompt specifically:
...
Three backgrounds live in `extras/gdm`:

| file | what it is |
|---|---|
| `nordic-login.png` | plain Polar Night gradient with a snowflake mark |
| `nordic-login-storm.png` | drawn storm — lightning, rain, hatched sea |
| `nordic-login-jormungandr.png` | artwork recoloured to the palette (default) |

Pick one with `LOGIN_BG` at the top of `install-nordic.sh`, or per-run with
`NORDIC_LOGIN_BG=nordic-login.png ./install-nordic.sh`. All three install as
`/usr/share/backgrounds/nordic-login.png`, so switching leaves nothing behind.

The Jörmungandr one is made by gradient-mapping the luminance of
`extras/gdm/jormungandr-source.jpg` onto the palette, then fading the middle
back into the plain gradient. Swap that source file to use different artwork —
anything roughly landscape works, it is cropped to 16:9 keeping the top. Two
values matter and are easy to get wrong: the calm radius must stay near
`0.235*w`, since the dialog only spans about `0.41–0.59w` and anything wider
reaches the serpent's head at `~0.35w` and washes out the subject; and the top
fade must stay light, because the head and the lit break in the cloud are both
in the upper third.

The storm variant is atmosphere only, with no figures. Drawing the serpent, the
longship and a breaking wave with cairo primitives was tried twice and produced
a blob, a bent pipe and a top hat — the method is good for gradients, hatching
